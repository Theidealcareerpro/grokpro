...
/** Root / Group / Value (re-export) */
const Select = SelectPrimitive.Root
const SelectGroup = SelectPrimitive.Group
const SelectValue = SelectPrimitive.Value

/** ---------- Trigger (guarded ref to prevent loops) ---------- */
...
const SelectTrigger = React.forwardRef<TriggerEl, TriggerProps>(
  ({ className, children, ...props }, forwardedRef) => {
    // Guard callback refs so they only propagate when the node identity changes
    const lastNode = React.useRef<TriggerEl | null>(null)
    const safeRef = React.useCallback(
      (node: TriggerEl | null) => {
        if (lastNode.current === node) return
        lastNode.current = node

        if (typeof forwardedRef === "function") {
          forwardedRef(node)
        } else if (forwardedRef) {
          ;(forwardedRef as React.MutableRefObject<TriggerEl | null>).current = node
        }
      },
      [forwardedRef]
    )
