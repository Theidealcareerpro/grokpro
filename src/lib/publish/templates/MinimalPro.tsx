    ...
    <main>
      <div className="nav">
        <div className="nav-inner">
          <div className="brand">
            {data?.photoDataUrl ? (
              <img src={data.photoDataUrl} alt={data.fullName || 'Avatar'} width={32} height={32}
                   style={{borderRadius: '999px', border: '1px solid #e2e8f0', objectFit: 'cover'}} />
            ) : <span style={{width:32,height:32,borderRadius:'999px',background:'#e2e8f0',display:'inline-block'}} aria-hidden/>}
            <span>{data?.fullName || 'Your Name'}</span>
          </div>
          <div className="link-row muted">
            <a href="#about">About</a><a href="#skills">Skills</a>
            <a href="#projects">Projects</a><a href="#certifications">Certifications</a><a href="#contact">Contact</a>
          </div>
        </div>
      </div>
      ...
      <section id="home" className="hero">
        <div className="container" style={{textAlign:'center'}}>
          ...
          <p className="lead">{data?.tagline || 'Focused. Minimal. Effective.'}</p>
      ...
      {data?.about && (
        <section id="about" className="section">
          <div className="container">
            ...
            <p className="card muted" style={{textAlign:'justify'}}>{data.about}</p>
          </div>
        </section>
      )}
      ...
      {skills.length > 0 && (
        <section id="skills" className="section" style={{background:'#fff'}}>
          <div className="container">
            ...
            <div className="grid">
              {skills.map((s, i) => <div className="card" key={i}><div>{s}</div></div>)}
            </div>
          </div>
        </section>
      )}
      ...
      <footer className="footer">© {new Date().getFullYear()} {data?.fullName || 'Your Name'} · Minimal Portfolio</footer>
    </main>
  );
}
