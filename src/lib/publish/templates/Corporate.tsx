// src/lib/publish/templates/CorporatePublish.tsx
import 'server-only';
import type { PortfolioData } from '@/lib/portfolio-types';

export default function Corporate({ data }: { data: PortfolioData }) {
  ...
  return (
    <div className="font-serif bg-gradient-to-b from-slate-50 to-slate-100 text-slate-800 min-h-screen antialiased">
          ...
          <div className="hidden md:flex items-center gap-6 font-medium">
            {['home', 'about', 'skills', 'projects', 'certifications', 'contact'].map((id) => (
              <a
                key={id}
                href={`#${id}`}
                className="relative px-1 py-2 outline-none transition text-slate-600 hover:text-[#0A1E4F] focus-visible:text-[#0A1E4F]"
              >
      ...
      >
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(60%_50%_at_50%_0%,#cbd5e1_0%,transparent_60%),radial-gradient(40%_40%_at_80%_10%,#00CFFF33_0%,transparent_70%),radial-gradient(45%_45%_at_20%_5%,#FF7AF533_0%,transparent_70%),linear-gradient(to_bottom,#ffffff,#f1f5f9)] animate-[gradientShift_14s_ease_in_out_infinite] bg-[length:160%_160%]"
        />
        ...
        <section id="skills" className="scroll-mt-24 bg-white px-6 py-20" aria-label="Skills">
          ...
          <div className="mx-auto max-w-3xl flex flex-col gap-6">
            {skills.map((s, i) => (
              ...
              >
                {/* generic skill glyph */}
                <svg className="h-7 w-7 text-[#0A1E4F]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <section id="projects" className="mx-auto max-w-3xl scroll-mt-24 px-6 py-20" aria-label="Projects">
          ...
          <div className="flex flex-col gap-6">
            {projects.map((p, i) => (
              ...
              >
                {/* list icon */}
                <svg className="h-8 w-8 flex-shrink-0 text-[#FF7AF5]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path d="M4 7h16M4 12h16M4 17h16" />
                </svg>
                <div>
                  <h3 className="text-xl font-semibold text-[#0A1E4F]">{p?.name?.trim() || `Project ${i + 1}`}</h3>
        ...
        <div className="flex flex-wrap justify-center gap-6 text-lg">
          {data?.email && (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-md px-3 py-2 text-[#0A1E4F] transition hover:text-[#00CFFF]"
            >
              {/* mail icon */}
      ...
      {/* Minimal global CSS to match animation behavior */}
      <style>{`
...
/** Small helper component to keep headings consistent with your preview */
